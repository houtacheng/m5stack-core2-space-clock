...
# Normalize navigation artwork by visible bounds, so unlike source canvases have
# the same apparent size on the 25-pixel bottom bar.
for source_name, output_name in (
    ...
    if source_path.exists():
        fit(Image.open(source_path).convert("RGBA"), (24, 24), 1).save(OUTPUT / output_name, optimize=True)
